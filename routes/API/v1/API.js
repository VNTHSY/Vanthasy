...
router.post("/api/auth/create-user", validateApiKey, async (req, res) => {
  try {
    ...
    const userExists = await db
      .get("users")
      .then(
        (users) => users && users.some((user) => user.username === username)
      );
...
router.post("/api/instances/deploy", validateApiKey, async (req, res) => {
  const { image, memory, cpu, ports, nodeId, name, user, primary } = req.body;
...
router.delete("/api/instance/delete", validateApiKey, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ Message: "The instance has successfully been deleted." });
  } catch (error) {
...
router.post("/api/getUserInstance", validateApiKey, async (req, res) => {
  ...
  const userExists = await db
    .get("users")
    .then((users) => users && users.some((user) => user.userId === userId));
...
router.post("/api/getInstance", validateApiKey, async (req, res) => {
  ...
  const instanceExists = await db
    .get("instances")
    .then((server) => server && server.some((server) => server.Id === id));
...
router.get("/api/name", validateApiKey, async (req, res) => {
  try {
    const name = (await db.get("name")) || "Overvoid";
    res.json({ name });
  } catch (error) {
...
async function deleteInstance(instance) {
  try {
    await axios.get(
      `http://Overvoid:${instance.Node.apiKey}@${instance.Node.address}:${instance.Node.port}/instances/${instance.ContainerId}/delete`
    );
